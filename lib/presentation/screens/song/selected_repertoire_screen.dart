      ...
      body: Consumer(
        builder: (context, ref, child) {
          ...
          return FutureBuilder<List<Song>>(
            future: Future.wait(
              currentRepertoire.songIds.map((id) => ref.read(songRepositoryProvider).getSongById(id)),
            ),
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) {
                ...
                return Center(child: Text('No tienes canciones agregadas al esquema.', style: GoogleFonts.robotoMono(
                  fontSize: 16
                ),));
              }

              final songs = snapshot.data!;
              final orderedSongs = currentRepertoire.songIds.map((id) => songs.firstWhere((song) => song.id == id)).toList();
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: [
            ...
            TextButton(
              onPressed: () async {
                await ref.read(repertoireNotifierProvider.notifier).removeSongFromRepertoire(
                  repertoire.id,
                  song.id,
                );
                if(context.mounted){
